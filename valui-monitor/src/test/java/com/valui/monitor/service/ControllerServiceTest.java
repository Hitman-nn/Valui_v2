...
class ControllerServiceTest {
    ...
    void addController_happyPath_returnsDto() {
        ...
        verify(planLimitFacade).debitForBkSlotIfNew(TG_ID, "XBET");
    ...
    void addController_unknownBookmaker_throws400() {
        ...
        assertThatThrownBy(() ->
            service.addController(
                new CreateControllerRequest("https://unknown-bk.com/line/1", null, null, false), TG_ID))
            .isInstanceOf(ValuiException.class)
    ...
    void muteController_setsFlag() {
        ...
        then(controllerPort).should().muteSubscription(cid, TG_ID);
    }
    ...
    void unmuteController_clearsFlag() {
        ...
        then(controllerPort).should().unmuteSubscription(cid, TG_ID);
    }

    // ── updateFilterRule ──────────────────────────────────────────────────────

    @Test
    @DisplayName("updateFilterRule: new filter — debits tokens then saves")
    void updateFilterRule_newFilter_debitsTokens() {
        UUID cid = UUID.randomUUID();
        ControllerEntity entity = controllerEntity(cid, XBET_URL, true);
        given(userService.findByTelegramId(TG_ID)).willReturn(Optional.of(user));
        given(controllerPort.findByIdAndUserId(cid, USER_ID)).willReturn(Optional.of(entity));
        ...
        verify(planLimitFacade).debitForControllerFilter(TG_ID);
        assertThat(entity.getFilterRule()).isEqualTo(".*Liverpool.*");
    }

    @Test
    @DisplayName("updateFilterRule: updating existing filter — skips token debit")
    void updateFilterRule_existingFilter_skipsDebit() {
        UUID cid = UUID.randomUUID();
        ControllerEntity entity = controllerEntity(cid, XBET_URL, true);
        ...
        verify(planLimitFacade, never()).debitForControllerFilter(TG_ID);
    }
