...
public class MonthlyTokenBillingScheduler {
    ...
    public void runMonthlyBilling() {
        ...
        List<UserEntity> activeUsers = userRepository.findAllByStatus(
            com.valui.common.domain.UserStatus.ACTIVE);

        int totalUsers = 0, totalGranted = 0, totalBkCharged = 0, totalFilterCharged = 0;

        for (UserEntity user : activeUsers) {
            try {
                BillingResult result = processUser(user, billingMonth);
    ...
    @Transactional
    public BillingResult processUser(UserEntity user, YearMonth billingMonth) {
        ...
        return new BillingResult(granted, bkCharged, filterCharged);
    }

    private void pauseBookmakerControllers(java.util.UUID userId, BookmakerType bookmaker) {
